run_heatmap_pipeline <- function(meeting_filter = function(dates, schedule) dates,
                                       run_label = "all meetings") {
  # =============================================
  # 1) Setup & libraries
  # =============================================
  ...
  cat(sprintf("\nProcessing heatmaps for %s.\n", run_label))
  meeting_dates <- sort(unique(as.Date(meeting_schedule$meeting_date)))
  meetings_to_process <- meeting_filter(meeting_dates, meeting_schedule)
  meetings_to_process <- sort(unique(as.Date(meetings_to_process)))
  meetings_to_process <- meetings_to_process[!is.na(meetings_to_process)]
  cat("Total meetings to process:", length(meetings_to_process), "\n\n")

  if (length(meetings_to_process) == 0) {
    cat("No meetings to process for this run.\n")
